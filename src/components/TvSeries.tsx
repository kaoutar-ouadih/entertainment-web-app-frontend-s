...
const TvSeries: React.FC<TvSeriesProps> = ({copyOfData, bookmarkedItems, setBookmarkedItems}) => {
  ...
  return (    
    <div className="px-7">
        <h1 className="text-white text-3xl font-extralight mb-7">TV Series</h1>
      <div className="flex gap-7 flex-wrap justify-between">
        { copyOfData.map((item)=>(
        <div key={item.title} className="min-w-[200px] basis-[210px]  lg:basis-[250px] mb-4">
            <div className="w-full rounded-md overflow-hidden relative mb-2 group/outer" >
                ...
                {/* bookMark */}
                <button onClick={()=>bookmark(item.id)} aria-label="bookmark" className="absolute top-4 right-4 w-9 h-9 rounded-full bg-darkBlue bg-opacity-40 flex justify-center items-center z-10 group/inner hover:bg-white">
